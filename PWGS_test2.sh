...
while [ $i -lt $len ]; do
  echo "$i: ${scaf_tot[$i]}"
  qsub -v min="$min",max="$max",min_qual="$min_qual",chr_pool="$chr_pool",theta="$theta",D="$D",n_threads="$n_threads",bam_in="$bam_in",scaffold=${scaf_tot[$i]},nome=$nome"_SNP_scaf/scaf"${scaf_num[$i]} -pe smp $n_threads -o $nome"_"${scaf_num[$i]}"_worker2.out" -hold_jid $nome"_PWGS_mergebam" -N $nome"worker2" PWGS_worker2.sh
